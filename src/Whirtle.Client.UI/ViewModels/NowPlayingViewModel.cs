...
public sealed partial class NowPlayingViewModel : ObservableObject
{
            ...
            _ = convergenceTask.ContinueWith(t =>
                {
                    ...
                    if (converged)
                        Log.Information(
                            "Clock synchronized to <{TargetMs:F3} ms in {ElapsedMs:F0} ms",
    ...
    private async Task RunServerAcceptLoopAsync(CancellationToken cancellationToken)
    {
        ...
        Log.Debug(
            "Server-initiated mode: listening on :{Port}{Path}",
        ...
        _ = convergenceTask.ContinueWith(t =>
            {
                ...
                if (converged)
                    Log.Information(
                        "Clock synchronized to <{TargetMs} ms in {ElapsedMs:F0} ms",
